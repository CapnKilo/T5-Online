...
Public DDS_SkillIcon() As DirectDrawSurface7
Public DDS_Face() As DirectDrawSurface7

Public DDS_Door As DirectDrawSurface7 ' singles
...
Public DDSD_SkillIcon() As DDSURFACEDESC2
...
Dim rec As DxVBLib.RECT
    ...
    If FileExist(App.Path & "\data files\graphics\door.bmp") Then Call InitDDSurf("door", DDSD_Door, DDS_Door)
...
Dim i As Long
    ...
    For i = 1 To NumSkillIcons
        Set DDS_SkillIcon(i) = Nothing
        ZeroMemory ByVal VarPtr(DDSD_SkillIcon(i)), LenB(DDSD_SkillIcon(i))
    Next
...
Dim i As Long, NpcNum As Long, partyIndex As Long
    ...
    If SkillBuffer > 0 Then
        If Skill(PlayerSkills(SkillBuffer)).CastTime > 0 Then
            ' lock to player
            ...
            ' calculate the width to fill
            barWidth = (timeGetTime - SkillBufferTimer) / ((Skill(PlayerSkills(SkillBuffer)).CastTime * 1000)) * sWidth
        ...
        Select Case Hotbar(i).sType
            ...
            Case 2 ' Skill
                With sRECT
                    ...
                    .Right = 32
                End With
                If Len(Skill(Hotbar(i).Slot).name) > 0 Then
                    If Skill(Hotbar(i).Slot).Icon > 0 Then
                        If DDS_SkillIcon(Skill(Hotbar(i).Slot).Icon) Is Nothing Then
                            Call InitDDSurf("Skillicons\" & Skill(Hotbar(i).Slot).Icon, DDSD_SkillIcon(Skill(Hotbar(i).Slot).Icon), DDS_SkillIcon(Skill(Hotbar(i).Slot).Icon))
                        End If
                        ' check for cooldown
                        For n = 1 To MAX_PLAYER_SkillS
                            If PlayerSkills(n) = Hotbar(i).Slot Then
                                ' has Skill
                            ...
                            End If
                        Next
                        Engine_BltToDC DDS_SkillIcon(Skill(Hotbar(i).Slot).Icon), sRECT, dRECT, frmMain.picHotbar, False
                    End If
                End If
        End Select
...
Public Sub BltPlayerSkills()
Dim i As Long, X As Long, Y As Long, Skillnum As Long, Skillicon As Long
...
Dim colour As Long
    ...
    For i = 1 To MAX_PLAYER_SkillS
        ...
        If Skillnum > 0 And Skillnum <= MAX_SkillS Then
            ...
            If Skillicon > 0 And Skillicon <= NumSkillIcons Then
                ...
                ' Load Skillicon if not loaded, and reset timer
                SkillIconTimer(Skillicon) = timeGetTime + SurfaceTimerMax

                If DDS_SkillIcon(Skillicon) Is Nothing Then
                    Call InitDDSurf("SkillIcons\" & Skillicon, DDSD_SkillIcon(Skillicon), DDS_SkillIcon(Skillicon))
                End If

                Engine_BltToDC DDS_SkillIcon(Skillicon), rec, positionRec, frmMain.picSkills, False
            End If
        End If
    Next
    ...
    Exit Sub
errorhandler:
    HandleError "BltPlayerSkills", "modDirectDraw7", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Public Sub BltDraggedSkill(ByVal X As Long, ByVal Y As Long)
Dim rec As RECT, positionRec As RECT
Dim Skillnum As Long, Skillpic As Long
    ...
    Skillnum = PlayerSkills(DragSkill)
    ...
    If Skillnum > 0 And Skillnum <= MAX_SkillS Then
        ...
        ' Load item if not loaded, and reset timer
        SkillIconTimer(Skillpic) = timeGetTime + SurfaceTimerMax

        If DDS_SkillIcon(Skillpic) Is Nothing Then
            Call InitDDSurf("Skillicons\" & Skillpic, DDSD_SkillIcon(Skillpic), DDS_SkillIcon(Skillpic))
        End If

        Engine_BltToDC DDS_SkillIcon(Skillpic), rec, positionRec, frmMain.picTempskill, False

        With frmMain.picTempskill
            .top = Y
            .Left = X
            .Visible = True
            .ZOrder (0)
...
Public Sub BltSkillDesc(ByVal Skillnum As Long)
Dim rec As RECT, positionRec As RECT
Dim Skillpic As Long

    ' If debug mode, handle error then exit out
    If Options.Debug = 1 Then On Error GoTo errorhandler
    
    frmMain.picSkillDescPic.Cls

    If Skillnum > 0 And Skillnum <= MAX_SkillS Then
        Skillpic = Skill(Skillnum).Icon

        If Skillpic <= 0 Or Skillpic > NumSkillIcons Then Exit Sub
        ...
        With positionRec
            ...
            .Right = 64
        End With
        Engine_BltToDC DDS_SkillIcon(Skillpic), rec, positionRec, frmMain.picSkillDescPic, False
    End If

    ' Error handler
    Exit Sub
errorhandler:
    HandleError "BltSkillDesc", "modDirectDraw7", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Dim dRECT As DxVBLib.RECT
    ...
    If iconnum < 1 Or iconnum > NumSkillIcons Then
        frmEditor_Skill.picSprite.Cls
        Exit Sub
    End If
    
    SkillIconTimer(iconnum) = timeGetTime + SurfaceTimerMax
    
    If DDS_SkillIcon(iconnum) Is Nothing Then
        Call InitDDSurf("SkillIcons\" & iconnum, DDSD_SkillIcon(iconnum), DDS_SkillIcon(iconnum))
    End If
    ...
    Call Engine_BltToDC(DDS_SkillIcon(iconnum), sRECT, dRECT, frmEditor_Skill.picSprite)
    
    ' Error handler
    Exit Sub
errorhandler:
    HandleError "EditorSkill_BltIcon", "modDirectDraw7", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
