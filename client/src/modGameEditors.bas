...
Dim SoundSet As Boolean
    ...
    With Item(EditorIndex)
        ...
        If frmEditor_Item.cmbType.ListIndex = ITEM_TYPE_CONSUME Then
            ...
            frmEditor_Item.scrlCastSkill.value = .CastSkill
            frmEditor_Item.chkInstant.value = .instaCast
        Else
            frmEditor_Item.fraVitals.Visible = False
        End If

        If (frmEditor_Item.cmbType.ListIndex = ITEM_TYPE_Skill) Then
...
Dim SoundSet As Boolean
    ...
    EditorIndex = frmEditor_Skill.lstIndex.ListIndex + 1
    ...
    End If
    ' add the array to the combo
    frmEditor_Skill.cmbSound.Clear
    frmEditor_Skill.cmbSound.AddItem "None."
    For i = 1 To UBound(soundCache)
        frmEditor_Skill.cmbSound.AddItem soundCache(i)
    Next
    ' finished populating
    ...
    With frmEditor_Skill
        ...
        .txtDesc.Text = Trim$(Skill(EditorIndex).Desc)
        .cmbType.ListIndex = Skill(EditorIndex).Type
        ...
        .cmbClass.ListIndex = Skill(EditorIndex).ClassReq
        ...
        If Skill(EditorIndex).IsAoE Then
            .chkAOE.value = 1
        Else
            .chkAOE.value = 0
        End If
        ...
        If .cmbSound.ListCount >= 0 Then
            For i = 0 To .cmbSound.ListCount
                If .cmbSound.List(i) = Trim$(Skill(EditorIndex).Sound) Then
                    .cmbSound.ListIndex = i
                    SoundSet = True
                    Exit For
                End If
    ...
    Exit Sub
errorhandler:
    HandleError "SkillEditorInit", "modGameEditors", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Dim i As Long
    ...
    Exit Sub
errorhandler:
    HandleError "SkillEditorOk", "modGameEditors", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Public Sub SkillEditorCancel()
    ...
    Exit Sub
errorhandler:
    HandleError "SkillEditorCancel", "modGameEditors", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Public Sub ClearChanged_Skill()
    ...
    ZeroMemory Skill_Changed(1), MAX_SkillS * 2 ' 2 = boolean length
    
    ' Error handler
    Exit Sub
errorhandler:
    HandleError "ClearChanged_Skill", "modGameEditors", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
