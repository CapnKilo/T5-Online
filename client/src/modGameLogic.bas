    ...
    Do While InGame
        ...
        If tmr10000 < Tick Then
            ...
            If NumSkillIcons > 0 Then
                For i = 1 To NumSkillIcons    'Check to unload surfaces
                    If SkillIconTimer(i) > 0 Then 'Only update surfaces in use
                        If SkillIconTimer(i) < Tick Then   'Unload the surface
                            Call ZeroMemory(ByVal VarPtr(DDSD_SkillIcon(i)), LenB(DDSD_SkillIcon(i)))
...
Dim Dir As Byte
    ...
    If DirUp Then
        ...
        ' Check the last direction matches up
        If LastDir <> GetPlayerDir(MyIndex) Then
           Call SendPlayerDir
           LastDir = GetPlayerDir(MyIndex)
        End If
...
Dim buffer As clsBuffer
    ...
    If SkillCD(SkillSlot) > 0 Then
        AddText "Cannot forget a Skill which is cooling down!", BrightRed
        Exit Sub
    End If
    
    ' dont let them forget a Skill which is buffered
    If SkillBuffer = SkillSlot Then
        AddText "Cannot forget a Skill which you are casting!", BrightRed
        Exit Sub
    End If
    
    If PlayerSkills(SkillSlot) > 0 Then
        Set buffer = New clsBuffer
        buffer.WriteLong CForgetSkill
        buffer.WriteLong SkillSlot
        SendData buffer.ToArray()
        Set buffer = Nothing
    Else
        AddText "No Skill here.", BrightRed
    End If
    
    ' Error handler
    Exit Sub
errorhandler:
    HandleError "ForgetSkill", "modGameLogic", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub

Public Sub CastSkill(ByVal SkillSlot As Long)
Dim buffer As clsBuffer
    ...
    If SkillCD(SkillSlot) > 0 Then
        AddText "Skill has not cooled down yet!", BrightRed
        Exit Sub
    End If
    
    If PlayerSkills(SkillSlot) = 0 Then Exit Sub

    ' Check if player has enough MP
    If GetPlayerVital(MyIndex, Vitals.MP) < Skill(PlayerSkills(SkillSlot)).MPCost Then
        Call AddText("Not enough MP to cast " & Trim$(Skill(PlayerSkills(SkillSlot)).name) & ".", BrightRed)
        Exit Sub
    End If
    ...
    If PlayerSkills(SkillSlot) > 0 Then
        If timeGetTime > Player(MyIndex).AttackTimer + 1000 Then
            If Player(MyIndex).Moving = 0 Then
                ...
                SkillBufferTimer = timeGetTime
            Else
                Call AddText("Cannot cast while walking!", BrightRed)
            End If
        End If
    Else
        Call AddText("No Skill here.", BrightRed)
    End If

    ' Error handler
    Exit Sub
errorhandler:
    HandleError "CastSkill", "modGameLogic", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Public Sub UpdateSkillWindow(ByVal Skillnum As Long, ByVal X As Long, ByVal Y As Long)
Dim i As Long
    ...
    If Y + frmMain.picSkillDesc.height > frmMain.ScaleHeight Then
        Y = frmMain.ScaleHeight - frmMain.picSkillDesc.height
    End If
    ...
    With frmMain
        ...
        .lblSkillName.Caption = Trim$(Skill(Skillnum).name)
        .lblSkillDesc.Caption = Trim$(Skill(Skillnum).Desc)
        BltSkillDesc Skillnum
    End With
    
    ' Error handler
    Exit Sub
errorhandler:
    HandleError "UpdteSkillWindow", "modGameLogic", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
...
Dim soundName As String
    ...
    Select Case entityType
        ...
        ' Skills
        Case SoundEntity.seSkill
            If entityNum > MAX_SkillS Then Exit Sub
            soundName = Trim$(Skill(entityNum).Sound)
        ' other
        Case Else
            Exit Sub
    End Select
