...
Sub OnDeath(ByVal index As Long)
    ...
    ' Clear Skill casting
    TempPlayer(index).SkillBuffer.Skill = 0
    TempPlayer(index).SkillBuffer.Timer = 0
    TempPlayer(index).SkillBuffer.target = 0
    TempPlayer(index).SkillBuffer.tType = 0
    Call SendClearSkillBuffer(index)
        ...
        If ResourceCache(GetPlayerMap(index)).ResourceData(Resource_num).ResourceState = 0 Then
            ...
            If Damage > 0 Then
                ...
                If ResourceCache(GetPlayerMap(index)).ResourceData(Resource_num).cur_health - Damage <= 0 Then
                    ...
                    Select Case Resource(Resource_index).QuestType
                        Case 0
                            Exit Sub
                        Case 1 ' start quest
                            Call StartQuest(index, Resource(Resource_index).QuestIndex)
                        Case 2 ' advance quest
                            Call AdvanceQuest(index, Resource(Resource_index).QuestIndex, Resource(Resource_index).QuestTask)
                    End Select
        ...
        Select Case Item(itemNum).Type
            ...
            Case ITEM_TYPE_Skill
                ...
                If n > 0 Then
                        ...
                        If i <= GetPlayerLevel(index) Then
                            ...
                            If i > 0 Then
                                ...
                                If Not HasSkill(index, n) Then
                                    ...
                                    Call SendAnimation(GetPlayerMap(index), Item(itemNum).Animation, 0, 0, TARGET_TYPE_PLAYER, index)
                                    Call TakeInvItem(index, itemNum, 0)
                                    Call PlayerMsg(index, "You feel the rush of knowledge fill your mind. You can now use " & Trim$(Skill(n).Name) & ".", BrightGreen)
                    ...
                    Else
                        Call PlayerMsg(index, "This Skill can only be learned by " & CheckGrammar(GetClassName(Skill(n).ClassReq)) & ".", BrightRed)
                    End If
                End If
