...
Sub AddChar(ByVal index As Long, ByVal Name As String, ByVal Sex As Byte, ByVal ClassNum As Long, ByVal Sprite As Long)
    ...
    If LenB(Trim$(Player(index).Name)) = 0 Then
        ...
        ' set start Skills
        If Class(ClassNum).startSkillCount > 0 Then
            For n = 1 To Class(ClassNum).startSkillCount
                If Class(ClassNum).StartSkill(n) > 0 Then
                    ' Skill exist?
                    If Len(Trim$(Skill(Class(ClassNum).StartSkill(n)).Name)) > 0 Then
                        Player(index).Skill(n) = Class(ClassNum).StartSkill(n)
                    End If
                End If
            Next
        End If
...
Sub LoadClasses()
    ...
    Dim startItemCount As Long, startSkillCount As Long
    Dim x As Long
    ...
    For i = 1 To Max_Classes
        ...
        ' how many starting Skills?
        startSkillCount = Val(GetVar(FileName, "CLASS" & i, "StartSkillCount"))
        If startSkillCount > 0 Then ReDim Class(i).StartSkill(1 To startSkillCount)
        
        ' loop for Skills
        Class(i).startSkillCount = startSkillCount
        If startSkillCount >= 1 And startSkillCount <= MAX_INV Then
            For x = 1 To startSkillCount
                Class(i).StartSkill(x) = Val(GetVar(FileName, "CLASS" & i, "StartSkill" & x))
            Next
        End If
    Next
...
Sub SaveClasses()
    ...
    For i = 1 To Max_Classes
        ...
        Next
        ' loop for Skills
        For x = 1 To UBound(Class(i).StartSkill)
            Call PutVar(FileName, "CLASS" & i, "StartSkill" & x, STR(Class(i).StartSkill(x)))
        Next
    Next
...
Sub SaveSkill(ByVal SkillNum As Long)
    Dim FileName As String
    Dim F As Long
    FileName = App.Path & "\data\Skills\Skills" & SkillNum & ".dat"
    F = FreeFile
    Open FileName For Binary As #F
...
Sub LoadSkills()
    ...
    For i = 1 To MAX_SkillS
        FileName = App.Path & "\data\Skills\Skills" & i & ".dat"
        F = FreeFile
        Open FileName For Binary As #F
        Get #F, , Skill(i)
        Close #F
...
Sub CheckSkills()
    ...
    For i = 1 To MAX_SkillS
        ...
        If Not FileExist("\Data\Skills\Skills" & i & ".dat") Then
            Call SaveSkill(i)
        End If
...
Sub ClearSkill(ByVal index As Long)
    Call ZeroMemory(ByVal VarPtr(Skill(index)), LenB(Skill(index)))
    Skill(index).Name = vbNullString
    Skill(index).LevelReq = 1 'Needs to be 1 for the Skill editor
    Skill(index).Desc = vbNullString
    Skill(index).Sound = "None."
End Sub
