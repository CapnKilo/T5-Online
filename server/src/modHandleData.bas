...
Public Sub InitMessages()
    ...
    HandleDataSub(CRequestEditSkill) = GetAddress(AddressOf HandleRequestEditSkill)
    HandleDataSub(CSaveSkill) = GetAddress(AddressOf HandleSaveSkill)
    ...
    HandleDataSub(CSkills) = GetAddress(AddressOf HandleSkills)
    ...
    HandleDataSub(CRequestSkills) = GetAddress(AddressOf HandleRequestSkills)
    ...
    HandleDataSub(CForgetSkill) = GetAddress(AddressOf HandleForgetSkill)
    ...
    HandleDataSub(CSwapSkillSlots) = GetAddress(AddressOf HandleSwapSkillSlots)
...
Sub HandlePlayerMove(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If TempPlayer(index).SkillBuffer.Skill > 0 Then
...
Sub HandleAttack(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If TempPlayer(index).SkillBuffer.Skill > 0 Then Exit Sub
...
' :::::::::::::::::::::::::::::
Sub HandleRequestEditSkill(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer
...
' :: Save Skill packet ::
' :::::::::::::::::::::::
Sub HandleSaveSkill(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    ' Save it
    Call SendUpdateSkillToAll(SkillNum)
    Call SaveSkill(SkillNum)
    Call AddLog(GetPlayerName(index) & " saved Skill #" & SkillNum & ".", ADMIN_LOG)
End Sub
...
Sub HandleSwapSkillSlots(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer
    ...
    If TempPlayer(index).SkillBuffer.Skill > 0 Then
        PlayerMsg index, "You cannot swap Skills whilst casting.", BrightRed
        Exit Sub
    End If
    
    For n = 1 To MAX_PLAYER_SkillS
        If TempPlayer(index).SkillCD(n) > timeGetTime Then
            PlayerMsg index, "You cannot swap Skills whilst they're cooling down.", BrightRed
            Exit Sub
        End If
    Next
...
Sub HandleRequestSkills(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    SendSkills index
End Sub
...
Sub HandleForgetSkill(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer
    ...
    If TempPlayer(index).SkillCD(Skillslot) > timeGetTime Then
        PlayerMsg index, "Cannot forget a Skill which is cooling down!", BrightRed
        Exit Sub
    End If
    
    ' dont let them forget a Skill which is buffered
    If TempPlayer(index).SkillBuffer.Skill = Skillslot Then
        PlayerMsg index, "Cannot forget a Skill which you are casting!", BrightRed
        Exit Sub
    End If
...
Sub HandleHotbarChange(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    Select Case sType
        ...
        Case 1 ' inventory
            ...
            End If
        Case 2 ' Skill
            If Slot > 0 And Slot <= MAX_PLAYER_SkillS Then
                If Player(index).Skill(Slot) > 0 Then
                    If Len(Trim$(Skill(Player(index).Skill(Slot)).Name)) > 0 Then
                        Player(index).Hotbar(hotbarNum).Slot = Player(index).Skill(Slot)
                        Player(index).Hotbar(hotbarNum).sType = sType
                    End If
                End If
            End If
    ...
    Select Case Player(index).Hotbar(Slot).sType
        ...
        Case 2 ' Skill
            For i = 1 To MAX_PLAYER_SkillS
                If Player(index).Skill(i) > 0 Then
                    If Player(index).Skill(i) = Player(index).Hotbar(Slot).Slot Then
                        BufferSkill index, i
                        Exit Sub
                    End If
                End If
...
Public Sub HandleConvEvent(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
...
Public Sub HandleCloseConv(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    TempPlayer(index).InChat = 0
End Sub

Public Sub HandleTargetUpdate(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
...
Public Sub HandleRequestQuestInfo(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
...
Public Sub HandleQuestDrop(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
