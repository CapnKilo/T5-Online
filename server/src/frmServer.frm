...
Dim x As Long
    ...
    For i = 1 To Player_HighIndex
        For x = 1 To MAX_QUESTS
            Player(i).Quest(x).TaskOn = 0
            Player(i).Quest(x).QuestStatus = 0
            Player(i).Quest(x).DataAmountLeft = 0
        Next
...
Private Sub cmdReloadSkills_Click()
Dim i As Long
    Call LoadSkills
    Call TextAdd("All Skills reloaded.")
    For i = 1 To Player_HighIndex
        If IsPlaying(i) Then
            SendSkills i
        End If
